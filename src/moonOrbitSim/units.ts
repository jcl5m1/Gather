...
// compact: if true, uses 3 significant digits (e.g. 1.23, 12.3, 123) instead of fixed 3 decimals
export function formatDistanceWithAstronomicalUnits(length: Length, compact: boolean = false): string {
    const kmValue = length.over(kilometers).value;
    const absKm = Math.abs(kmValue);

    const format = (val: number): string => {
        if (compact) {
            // Use toPrecision(3) for 3 significant digits
            // parseFloat cleanup removes trailing zeros if toPrecision leaves them unnecessary (though for 3 sig figs we usually want them if they are significant)
            // Actually, user wants "total of 3 digits around the decimal". 
            // toPrecision(3) gives exactly that.
            // We convert to Number and back to string to remove potential scientific notation for small numbers if we want, 
            // but for "1.23, 12.3, 123" toPrecision(3) returns strings like "1.23", "12.3", "123".
            // However, 100 -> "1.00e+2" with toPrecision(3)? No, 100 is 3 sig figs? 
            // 100..toPrecision(3) is "100". 
            // 1234..toPrecision(3) is "1.23e+3".
            // Since we scale units, we shouldn't hit huge numbers often, but if we do, scientific is probably fine or we let the unit scaler handle it.
