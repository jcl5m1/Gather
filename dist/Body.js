...
export class Body {
    constructor(id, name, color, radius, mass, shape, parent, attached, position, velocity, angularVelocity) {
        this.id = id;
        this.name = name;
        this.radius = radius;
        this.mass = mass;
        ...
        if (this.parent) {
            const parentObject = globals.bodies.find(o => o.id === this.parent);
            if (parentObject) {
    ...
    static fromJSON(json) {
        return new Body(json.id, json.name, json.color, json.radius, json.mass, json.shape, json.parent, json.attached, new THREE.Vector3(parseFloat(json.position.x), parseFloat(json.position.y), parseFloat(json.position.z)), new THREE.Vector3(parseFloat(json.velocity.x), parseFloat(json.velocity.y), parseFloat(json.velocity.z)), new THREE.Vector3(parseFloat(json.angularVelocity.x), parseFloat(json.angularVelocity.y), parseFloat(json.angularVelocity.z)));
    }
    toJSON() {
        return {
            id: this.id,
