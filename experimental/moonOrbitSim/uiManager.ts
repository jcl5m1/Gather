                    ...
                    if (focusedBodyName) {
                         ...
                         if (focusedBody && focusedBody.target === body) {
                            ...
                            if (result) {
                                ...
                                transferTrajectory.setTransferResult(result);
                                // Set start/end times
                                transferTrajectory.setTimes(currentTime, currentTime + result.timeOfFlight, result.startTime);
                                ...
                                const startTimeMeasure = Measure.of(currentTime + result.startTime, seconds);
                                
                                transferTrajectory.calculateFromState(posMeasure, velMeasure, centralMassMeasure, startTimeMeasure);

                                console.log("Transfer Trajectory:", posMeasure.getVector3(), 
                                velMeasure.getVector3(), startTimeMeasure.value);
                                ...
                                // compute the departure burn and arrival burn times based on a fixed acceleration
                                const fixedAccel = 0.0001; // km/s^2
                                const departureBurnTime = result.deltaV1 / fixedAccel;
                                const arrivalBurnTime = result.deltaV2 / fixedAccel;
                                
                                // Store burn times in the transfer trajectory
                                transferTrajectory.setBurnTimes(departureBurnTime, arrivalBurnTime);
                                
                                //compute the start of the departure burn and arrival burn
                                const departureBurnStartTime = currentTime + result.startTime - departureBurnTime/2;
                                const departureBurnEndTime = departureBurnStartTime + departureBurnTime;
                                const arrivalBurnStartTime = currentTime + result.timeOfFlight - arrivalBurnTime/2;
                                const arrivalBurnEndTime = arrivalBurnStartTime + arrivalBurnTime;

                                //add markers to the transfer trajectory using the start trajectory us the departure start time
                                const departureBurnStartPos = focusedBody.getTrajectory().getBezierPosition(departureBurnStartTime);
                                const departureBurnEndPos = transferTrajectory.getBezierPosition(departureBurnEndTime);
                                const arrivalBurnStartPos = transferTrajectory.getBezierPosition(arrivalBurnStartTime);
                                const arrivalBurnEndPos = focusedBody.target.getTrajectory().getBezierPosition(arrivalBurnEndTime);
                                
                                console.log("Time of Flight:", result.timeOfFlight);
                                console.log("Departure Burn Time:", departureBurnTime);
                                console.log("Departure Burn Start Time:", departureBurnStartTime);
                                console.log("Departure Burn End Time:", departureBurnEndTime);
                                console.log("Arrival Burn Time:", arrivalBurnTime);
                                console.log("Arrival Burn Start Time:", arrivalBurnStartTime);
                                console.log("Arrival Burn End Time:", arrivalBurnEndTime);

                                if (departureBurnStartPos) transferTrajectory.addMarker(departureBurnStartPos, 0x00ff00);
                                if (departureBurnEndPos) transferTrajectory.addMarker(departureBurnEndPos, 0xff0000);
                                if (arrivalBurnStartPos) transferTrajectory.addMarker(arrivalBurnStartPos, 0x00ff00);
                                if (arrivalBurnEndPos) transferTrajectory.addMarker(arrivalBurnEndPos, 0xff0000);

                                // Add Bezier burn arcs with velocity-based control points
                                // Departure burn arc: from start orbit to transfer trajectory
                                if (departureBurnStartPos && departureBurnEndPos) {
                                    const departureBurnStartVel = focusedBody.getTrajectory().getBezierState(departureBurnStartTime, { calcVelocity: true }).velocity;
                                    const departureBurnEndVel = transferTrajectory.getBezierState(departureBurnEndTime, { calcVelocity: true }).velocity;
                                    
                                    if (departureBurnStartVel && departureBurnEndVel) {
                                        transferTrajectory.addArc(
                                            departureBurnStartPos,
                                            departureBurnEndPos,
                                            departureBurnStartVel,
                                            departureBurnEndVel,
                                            0x00ffff, // Cyan color for departure burn arc
                                            10000 // Curvature scale (10 * velocity)
                                        );
                                    }
                                }

                                // Arrival burn arc: from transfer trajectory to target orbit
                                if (arrivalBurnStartPos && arrivalBurnEndPos) {
                                    const arrivalBurnStartVel = transferTrajectory.getBezierState(arrivalBurnStartTime, { calcVelocity: true }).velocity;
                                    const arrivalBurnEndVel = focusedBody.target.getTrajectory().getBezierState(arrivalBurnEndTime, { calcVelocity: true }).velocity;
                                    
                                    if (arrivalBurnStartVel && arrivalBurnEndVel) {
                                        transferTrajectory.addArc(
                                            arrivalBurnStartPos,
                                            arrivalBurnEndPos,
                                            arrivalBurnStartVel,
                                            arrivalBurnEndVel,
                                            0xff00ff, // Magenta color for arrival burn arc
                                            10 // Curvature scale (10 * velocity)
                                        );
                                    }
                                }
                                 ...
                                 if (plotWithRes) {
                                     ...
                                     plot.setAnimationPosition(refinedResult ? refinedResult.startDelay : result.startTime);
